/* 
  Music Player with Pan Percussion Filter
  FilteredPlayerApp.java
  FilterGUI.java
  jp/kmgoto/music/*.java

  Input file: PCM 44.1kHz, 16bitLE, Stereo (*.wav)
              or MP3
  Output: Audio or file
...
// open log window (right)
  ...
  private void startSave(File inputFile){
     try {
        ...
        player = new FilteredPlayer(inputFile, null, filter);
...
// Main Loop
       ...
       while((nread = ais.read(buf)) > 0 && running) {
         ...
         filter.apply(chunk); // apply filter
         ...
         if (buffer.size() < 44100*4) continue; 
          ...
          if (sline == null){ 
             ...
             nwritten = sline.write(outbytes,0,44100*4);
       ...
       if (sline == null) {
           ...
           nwritten = sline.write(outbytes,0,outbytes.length);
       }
  ...
  public static void main(String[] args) throws Exception {
    ...
    File outputFile = new File(args[0] + "-nodrums.wav");
    File rawFile = new File(args[0] + "-nodrums.raw");
     AdvancedFilter filter = new AdvancedFilter(44100f,4096,21);
     FilteredPlayer player 
         = new FilteredPlayer(inputFile,null, filter);
     player.setGain(100);
