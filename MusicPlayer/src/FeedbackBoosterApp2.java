...
public class FeedbackBoosterApp2 extends JFrame 
  ...
  private FeedbackBoosterPlayer2 player;
...
//  private static final double minFIndex = Math.log(11/440.0)/Math.log(2.0);
  ...
  FeedbackBoosterApp2() throws Exception {
...
//    add(panel3, BorderLayout.SOUTH);
  ...
  public synchronized void actionPerformed(ActionEvent event) {
   ...
   if (obj == setOutputPortCombo){
     ...
     if (inputPort != null && outputPort != null){
        ...
        player = new FeedbackBoosterPlayer2(inputPort, outputPort);
  ...
  public static void main(String[] args) throws Exception {
    ...
    if (args.length == 0) new FeedbackBoosterApp2();
    else FeedbackBoosterApp2.main(args);
    // System.exit(0);
  }

}

class FeedbackBoosterPlayer2 implements Runnable {
  ...
  private PitchFinderBuffered pitch; 
  private BiquadEQ eqBase, eqOct;
  private Compressor comp;
  private FeedbackBoosterApp2 parent;
  private int processSize;

  FeedbackBoosterPlayer2(Mixer.Info input, Mixer.Info output){
    ...
    processSize = 512;
    // affects processing delay 
    // 128/44100 = 2.9 msec , 256, 512, 1024 etc.
    ...
    int shift = processSize;
    int pitchWindow = 1024;
    pitch = new PitchFinderBuffered(44100f,pitchWindow,shift);
    ...
    setOctMix(0f);
    setCompGain(0f);
    comp = new Compressor(32767); // 16bit max value
  ...
  public void setParent(FeedbackBoosterApp2 obj){parent = obj;}
  ...
  public void run() {
    ...
    try {
       ...
       while((nread = iline.read(buf,0,buf.length)) > 0 && running) {
          ...
          // process effect here
            if (autoEQ){
              pitch.putSamples(floatSamples);
              // float tmp = pitch.findPitch(floatSamples);
              float tmp = pitch.findPitch();
              if (tmp > 0){
