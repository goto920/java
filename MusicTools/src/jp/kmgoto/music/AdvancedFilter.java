...
public class AdvancedFilter implements Serializable {
   ...
   public String getValue(float pan, float freq){
     int panIndex  = 
        Math.min(
              filterArray.length-1, 
              Math.max(0, (int) (panSplits*(pan + 1f)/2f))
        );
     int fIndex  = 
         Math.min(
              filterArray[0].length-1, 
              Math.max(0, (int) (freq*windowSize/samplingRate))
         );
     return filterArray[panIndex][fIndex];
   }
