...
public class PitchFinderBuffered extends BufferedEffector {

  private float samplingRate; 
  private int windowSize, shift;

  public PitchFinderBuffered(float samplingRate, int windowSize, int shift){
    super(1, samplingRate); // channel = 1
...
//  public float findPitch(float[] samples){
  public float findPitch(){
    ...
    if (inputBuffer.size() < windowSize) return -1f;

    float[] samples = new float[windowSize];
    for (int i=0; i < windowSize; i++) samples[i] = inputBuffer.get(i);

    float[] diff = calcNormSqDiff(samples);
    int index = getMaximaIndex(diff); 

    inputBuffer.subList(0, shift).clear();
...
/*
 @Override??
  protected void putSamples(float[] data, int len){}
  public float[] getSamples();
  public void flush();
*/
 @Override
  protected void process(){} // do nothing
 @Override
